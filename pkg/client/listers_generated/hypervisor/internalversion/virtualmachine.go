...
type VirtualMachineLister interface {
	...
	List(selector labels.Selector) (ret []*hypervisor.VirtualMachine, err error)
...
// List lists all VirtualMachines in the indexer.
func (s *virtualMachineLister) List(selector labels.Selector) (ret []*hypervisor.VirtualMachine, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*hypervisor.VirtualMachine))
	})
	return ret, err
}
...
type VirtualMachineNamespaceLister interface {
	...
	Get(name string) (*hypervisor.VirtualMachine, error)
...
// List lists all VirtualMachines in the indexer for a given namespace.
func (s virtualMachineNamespaceLister) List(selector labels.Selector) (ret []*hypervisor.VirtualMachine, err error) {
...
// Get retrieves the VirtualMachine from the indexer for a given namespace and name.
func (s virtualMachineNamespaceLister) Get(name string) (*hypervisor.VirtualMachine, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(hypervisor.Resource("virtualmachine"), name)
	}
	return obj.(*hypervisor.VirtualMachine), nil
}
